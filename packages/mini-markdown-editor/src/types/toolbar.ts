...
export interface ToolbarContextValues {
  toolbars: ToolbarItem[]; // 当前所有工具栏项
  addToolbar?: (item: ToolbarItem) => void; // 动态添加
  removeToolbar?: (type: ToolbarType) => void; // 按类型移除
  updateToolbar?: (type: ToolbarType, config: Partial<ToolbarItem>) => void; // 更新配置
  reorderToolbars?: (newOrder: ToolbarType[]) => void; // 重新排序
}
...
// 类型保护函数
export const isBaseToolbarType = (
  type: ToolbarType
): type is BaseToolbarType => {
...
// 更新验证函数以包含快捷键验证
export const validateToolbarItem = (
  item: ToolbarItem
): ToolbarValidationResult => {
  const errors: string[] = [];
