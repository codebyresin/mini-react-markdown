...
import { type ToolbarContextValues, type ToolbarItem } from "@/types/toolbar";
...
const processToolbars = (
  ...
  if (excludeTools?.length) {
    processedToolbars = processedToolbars.filter(
      (item) => !excludeTools.includes(item.type)
    );
  }
...
export const ToolbarProvider = React.memo<ToolbarProviderProps>(
