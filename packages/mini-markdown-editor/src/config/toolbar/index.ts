...
class ToolbarConfig extends BaseClass {
  ...
  // 修改特定工具栏项的功能
  public updateToolbarItem(
    type: ToolbarType,
    partialToolbarItem: Partial<ToolbarItem>
  ): void {
    try {
      this.checkDestroyed();
  ...
  // 交换工具栏位置（一对一）
  public swapToolbarsPosition(
    firstType: ToolbarType,
    secondType: ToolbarType
  ): void {
    try {
      this.checkDestroyed();
  ...
  public registerMethod(
    ...
    try {
      ...
      if (this.plugins.has(name) && !override) {
        throw new Error(
          `Method "${name}" already exists. Set override to true to replace it.`
        );
      }
