...
class ScrollSynchronizer {
  ...
  private getValidPreviewNodes(previewView: HTMLElement): Element[] {
    return Array.from(previewView.childNodes).filter((node: ChildNode) => {
      ...
      return !(
        (element.clientHeight === 0 && node.nodeName === "P") ||
        node.nodeType === 3
      );
    }) as Element[];
  }
  ...
  private isValidLineNumber(
    ...
    return (
      ...
      lineNumber <= editorView.state.doc.lines
    );
  }
  ...
  private getScrollElements(
    ...
    const scrollElement =
      source === "editor" ? editorView.scrollDOM : previewView;
    const targetElement =
      source === "editor" ? previewView : editorView.scrollDOM;
  ...
  private isScrolledToBottom(element: Element): boolean {
    return (
      element.scrollTop +
        ...
        ScrollSynchronizer.BOTTOM_THRESHOLD >=
  ...
  private scrollToBottom(targetElement: Element): void {
    ...
    const animate = (timestamp: number, startTime?: number) => {
      ...
      const percent = Math.min(
        progress / ScrollSynchronizer.SCROLL_ANIMATION_DURATION,
        1
      );
  ...
  private performProportionalScroll(
    ...
    const sourceList =
      source === "editor" ? this.editorElementList : this.previewElementList;
    const targetList =
      source === "editor" ? this.previewElementList : this.editorElementList;
  ...
  private calculateScrollPosition(
    ...
    const ratio = Math.max(
      0,
      Math.min(1, (scrollTop - sourceList[index]) / sourceDistance)
    );
...
export const handleEditorScroll = ({
  editorView,
  previewView,
}: InstancesType): void => {
...
export const handlePreviewScroll = ({
  editorView,
  previewView,
}: InstancesType): void => {
